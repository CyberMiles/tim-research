...
The Whitelist contract is an interesting example of inheritance. If you read https://github.com/bancorprotocol/contracts/blob/master/solidity/contracts/utility/Whitelist.sol[the source code] you will only see a few functions. However, as mentioned above the Whitelist contract does inherits from Utils and Owned and therefore the deployed Whitelist contract intance offers functions as per the rules of multiple inheritance. The true functionality of the Whitelist contract can therefore be viewed in the compiled contract's ABI file; the compiler inherits and makes available, to the base contract, all of the appropriate functions, modifiers and so on.
