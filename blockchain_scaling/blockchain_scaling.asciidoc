...
== Criteria
There are criteria which a second layer solution must meet in order to be a valid solution to blockchain scalability.
Firstly - transaction malleability.
Secondly - the ability to support multisig smart contracts
Thirdly - the ability to support timebased locks

== Goal
To allow the exchange of value to occur between two or more parties without the need for a trusted third party.

Pros of Plasma::
* Scaling by providing blockchains within blockchains

Cons of Plasma::
* Having to publish Plasma root headers to the main chain (low value - pending transactions?)
