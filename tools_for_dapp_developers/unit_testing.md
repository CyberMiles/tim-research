...
## Why a new way - there are others out there
Other tools such as [hevm](https://github.com/dapphub/dapptools/tree/master/src/hevm), [doppelganger](https://getdoppelganger.io/) and [ethtoolbox](https://eth-toolbox.com/) do offer unit testing operations, however I have learned over the years that if possible testing must take place as close to the action as possible. There are two reasons for this. Firstly code is idempotent; making the same calls generates the same results and encapsulating testdata and code in a sterile environment will generally yield the same result. Secondly, you are not just testing the code. There must be as many opportunities for every possible real-world scenario to be present during testing. Part of the testing process needs to involve real-world latency, timeouts, connectivity issues and more. This way you can be prepared for anything.
